// Package cli builds the devdocs command tree on top of the devdocs library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: devdocs.DefaultConfig()}

	root := &cobra.Command{
		Use:   "devdocs",
		Short: "Browse DevDocs.io programming documentation",
		Long: `devdocs reads documentation from DevDocs.io, which aggregates programming docs
for hundreds of languages, frameworks, and tools. The DevDocs.io API is open
and requires no key. It returns records as table, JSON, JSONL, CSV, TSV, or URLs.

devdocs is an independent tool and is not affiliated with DevDocs.io.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = devdocs.NewClient(a.cfg)
	return nil
}
