...
describes what users get. Pick a format with `-o`, or let devdocs choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
devdocs <command> -o table   # aligned columns for reading
devdocs <command> -o jsonl   # one JSON object per line, for piping
devdocs <command> -o json    # a single JSON array
devdocs <command> -o csv     # spreadsheet friendly
devdocs <command> -o tsv     # tab-separated
devdocs <command> -o url     # just the URL column
devdocs <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
devdocs <command> --fields id,title,url
```
...
```bash
devdocs <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
devdocs <command>            # a table, because this is a terminal
devdocs <command> | wc -l    # JSONL, because this is a pipe
```
